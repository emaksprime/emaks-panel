...
class TechnicalServiceTechnicianImportApplyTest extends TestCase
{
        ...
        $this->actingAs($this->admin())
            ->post('/api/technical-service/technicians/import-apply', [
                'file' => $this->csvUpload('stale.csv', [
                    ...
                    ['Değişen Usta', '0532 111 22 33', 'Sentetik Sehir 026', 'Adres'],
                ]),
                'preview_hash' => $preview['preview_hash'],
                'confirmation_text' => TechnicianImportApplyService::CONFIRMATION_TEXT,
                'selected_row_numbers' => [2],
    ...
    public function test_import_apply_rejects_more_than_50_rows(): void
    {
        ...
        for ($i = 1; $i <= 51; $i++) {
            $rows[] = ["PR88 Import {$i}", '0532 111 '.str_pad((string) $i, 4, '0', STR_PAD_LEFT), 'Sentetik Sehir 026', 'Adres'];
        }
    ...
    public function test_import_apply_updates_existing_by_phone_and_preserves_coordinates_by_default(): void
    {
        ...
        $rows = [
            ...
            ['PR88 Existing', '0532 111 22 33', 'Sentetik Sehir 031', 'Yeni adres', '39.0000000', '28.0000000'],
        ];
        ...
        $this->assertSame('Sentetik Sehir 031', $fresh->city);
    ...
    public function test_import_apply_links_existing_partner_by_mikro_cari_and_does_not_create_missing_partner(): void
    {
        ...
        $rows = [
            ...
            ['PR88 Link Usta', '0532 111 22 33', 'Sentetik Sehir 026', 'Adres', '320.CLG.PARTNER'],
            ['PR88 Missing Partner', '0532 111 22 44', 'Sentetik Sehir 026', 'Adres', '320.CLG.MISSING'],
        ];
    ...
    public function test_import_apply_keeps_distinct_synthetic_records_separate(): void
    {
        ...
        $existingTechnician = TechnicalServiceTechnician::query()->create([
            ...
            'phone_e164' => $existingFixture['phone_e164'],
        ...
        $rows = [
            ...
            [
                $partnerFixture['name'],
                $partnerFixture['phone_e164'],
                $partnerFixture['city'],
                $partnerFixture['address'],
                $partnerFixture['cari_code'],
            ],
        ];
        $preview = $this->preview('synthetic-distinct.csv', $rows);

        $this->actingAs($this->admin())
            ->post('/api/technical-service/technicians/import-apply', [
                'file' => $this->csvUpload('synthetic-distinct.csv', $rows),
                'preview_hash' => $preview['preview_hash'],
                'confirmation_text' => TechnicianImportApplyService::CONFIRMATION_TEXT,
                'selected_row_numbers' => [2],
            ])
        ...
        $this->assertSame(1, B2BPartner::query()->where('mikro_cari_kodu', $partnerFixture['cari_code'])->count());
    ...
    private function validRows(): array
    {
        return [
            ['Ad Soyad', 'Telefon', 'Şehir', 'Adres', 'Latitude', 'Longitude'],
            ['PR88-IMPORT-FIXTURE Yeni Usta', '0532 111 22 33', 'Sentetik Sehir 026', 'Fixture adres', '38.4237000', '27.1428000'],
        ];
    }
