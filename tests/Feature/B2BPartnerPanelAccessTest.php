...
class B2BPartnerPanelAccessTest extends TestCase
{
    ...
    public function test_super_admin_can_see_all_partners(): void
    {
        ...
        $dealer = $this->partner(['partner_type' => B2BPartner::TYPE_DEALER, 'display_name' => 'Sentetik Sehir 004 / SYNTHETIC Bayi']);
        $locksmith = $this->partner(['partner_type' => B2BPartner::TYPE_LOCKSMITH, 'display_name' => 'Sentetik Sehir 004 / SYNTHETIC Çilingir']);
    ...
    public function test_b2b_partner_search_finds_partner_by_linked_technician_name(): void
    {
        ...
        $technicianFixture = TechnicalServiceSyntheticDataFactory::locksmith(27);
        $partner = $this->partner([
            'partner_type' => B2BPartner::TYPE_DEALER,
            'capabilities' => [B2BPartner::TYPE_DEALER, B2BPartner::TYPE_LOCKSMITH],
            'display_name' => $partnerFixture['display_name'],
            'mikro_cari_kodu' => $partnerFixture['cari_code'],
            'city' => $partnerFixture['city'],
            'latitude' => '39.9111158',
            'longitude' => '32.8607935',
        ]);
        $technician = $this->technician([
            'name' => $technicianFixture['name'],
            'first_name' => $technicianFixture['first_name'],
            'last_name' => $technicianFixture['last_name'],
        ...
        $this->actingAs($admin)
            ->getJson('/api/b2b/partners?partner_type=locksmith&search=Sentetik')
            ->assertOk()
            ->assertJsonCount(1, 'items')
            ->assertJsonPath('items.0.id', $partner->id)
            ->assertJsonPath('items.0.display_name', $partnerFixture['display_name'])
            ->assertJsonPath('items.0.city', $partnerFixture['city'])
            ->assertJsonPath('items.0.linked_technicians.0.technician.name', $technicianFixture['name'])
            ->assertJsonPath('items.0.linked_technicians.0.technician.city', $technicianFixture['city']);

        $this->assertSame($technicianFixture['city'], $technician->fresh()->city);
    }
    ...
    public function test_b2b_locksmith_filter_includes_partner_with_linked_active_technician(): void
    {
        ...
        $technician = $this->technician(['name' => 'Filtre Ustası', 'city' => 'Sentetik Sehir 026']);
        B2BPartnerTechnician::query()->create([
            'partner_id' => $partner->id,
            'technical_service_technician_id' => $technician->id,
            'relationship_type' => 'contracted_technician',
        ...
        $this->actingAs($admin)
            ...
            ->assertJsonPath('items.0.linked_technicians.0.technician.city', 'Sentetik Sehir 026');
    }
    ...
    public function test_partner_can_have_multiple_active_technicians_without_replacing_existing_link(): void
    {
        ...
        $SyntheticPerson009 = $this->technician([
            'name' => 'SyntheticPerson009 Sentetik Sehir 004 / SYNTHETIC',
            'city' => 'Sentetik Sehir 004 / SYNTHETIC',
            'mikro_cari_kodu' => '320.CLG.BAHAT',
            'cari_code' => '320.CLG.BAHAT',
        ]);
        $SyntheticPerson072 = $this->technician([
            'name' => 'SyntheticPerson072 Izmir',
            'city' => 'Izmir',
            'mikro_cari_kodu' => '320.CLG.BAHAT',
            'cari_code' => '320.CLG.BAHAT',
        ]);
        ...
        $this->actingAs($admin)
            ->postJson("/api/b2b/partners/{$partner->id}/technicians", [
                'technical_service_technician_id' => $SyntheticPerson009->id,
                'is_primary' => true,
            ])
            ->assertCreated()
            ->assertJsonCount(1, 'items')
            ->assertJsonPath('partner.technical_service_technician_id', $SyntheticPerson009->id);

        $this->actingAs($admin)
            ->postJson("/api/b2b/partners/{$partner->id}/technicians", [
                'technical_service_technician_id' => $SyntheticPerson072->id,
            ])
            ->assertCreated()
            ->assertJsonCount(2, 'items')
            ->assertJsonPath('partner.technical_service_technician_id', $SyntheticPerson009->id);
    ...
    public function test_technician_lookup_empty_and_search_filters_active_technicians(): void
    {
        $admin = $this->userWithRole('admin', true);
        $technician = $this->technician([
            'name' => 'Lookup SyntheticPerson066 Usta',
            'phone' => '+905551234567',
        ...
        foreach (['SyntheticPerson066', '5551234567', 'istanbul', '320.CLG.LOOKUP'] as $search) {
            $this->actingAs($admin)
    ...
    public function test_portal_service_job_scope_uses_all_active_linked_technicians(): void
    {
        ...
        TechnicalServiceRequest::query()->create([
            ...
            'product_name' => 'Test SyntheticPerson038',
            'service_type' => 'Montaj',
            'technical_service_technician_id' => $first->id,
    ...
    {
        $admin = $this->userWithRole('admin', true);
        $fixture = TechnicalServiceSyntheticDataFactory::locksmith(4);
        $partner = $this->partner([
            'partner_type' => B2BPartner::TYPE_DEALER,
            'partner_code' => $fixture['cari_code'].'-EDIT',
            'display_name' => $fixture['display_name'],
            'mikro_cari_kodu' => $fixture['cari_code'].'-EDIT',
            'capabilities' => [B2BPartner::TYPE_DEALER],
        ]);
    ...
    public function test_locksmith_technician_lookup_searches_address_and_returns_full_payload(): void
    {
        ...
        $technician = $this->technician([
            ...
            'technician_type' => 'locksmith',
            'active' => true,
            'address' => 'SyntheticPerson084 Sokak No 5',
        ...
        $this->actingAs($admin)
            ->getJson('/api/b2b/locksmith-technicians?search=SyntheticPerson084')
            ->assertOk()
            ->assertJsonCount(1, 'items')
            ->assertJsonPath('items.0.id', $technician->id)
            ->assertJsonPath('items.0.address', 'SyntheticPerson084 Sokak No 5')
            ->assertJsonPath('items.0.city', 'Sentetik Sehir 009')
    ...
    public function test_partner_list_filters_search_type_active_and_city(): void
    {
        ...
        $this->partner([
            'partner_type' => B2BPartner::TYPE_DEALER,
            'display_name' => 'Sentetik Sehir 004 / SYNTHETIC Filtre Bayi',
        ...
        $this->partner([
            'partner_type' => B2BPartner::TYPE_LOCKSMITH,
            'display_name' => 'Sentetik Sehir 021 Filtre Çilingir',
        ...
        $this->actingAs($admin)
            ->getJson('/api/b2b/partners?partner_type=dealer&active=1&city=Sentetik Sehir 004 / SYNTHETIC&search=Filtre')
            ->assertOk()
            ->assertJsonCount(1, 'items')
            ...
            'https://n8n.test/*' => Http::response([
                ...
                'rows' => [
                    [
                        ...
                        'firma_unvani' => 'Sentetik Sehir 004 / SYNTHETIC Bayi A.S.',
                        'grup' => 'BAYI',
    ...
    public function test_cari_control_search_groups_child_cari_accounts_under_parent_candidate(): void
    {
        ...
        Http::fake([
            'https://n8n.test/*' => Http::response([
                ...
                'rows' => [
                    [
                        'musteri_kodu' => 'SYNTH-CARI-019',
                        'firma_unvani' => 'KEYSWORLD GUVENLIK SyntheticPerson039',
                        'grup' => 'BAYI',
                        'city' => 'Istanbul',
                    ],
                    [
                        'musteri_kodu' => 'SYNTH-CARI-019.KONSINYE',
                        'firma_unvani' => 'KEYSWORLD GUVENLIK SyntheticPerson039 KONSINYE',
                        'grup' => 'BAYI',
                    ],
                    [
                        'musteri_kodu' => 'SYNTH-CARI-019.TEŞHIR',
                        'firma_unvani' => 'KEYSWORLD GUVENLIK SyntheticPerson039 TESHIR',
                        'grup' => 'BAYI',
                    ],
                    [
                        'musteri_kodu' => 'SYNTH-CARI-019.PROJE',
                        'firma_unvani' => 'KEYSWORLD GUVENLIK SyntheticPerson039 PROJE',
                        'grup' => 'BAYI',
                    ],
                    [
        ...
        $this->actingAs($admin)
            ...
            ->assertJsonPath('candidates.0.mikro_cari_kodu', 'SYNTH-CARI-019')
            ->assertJsonPath('candidates.0.search_match', 'child')
            ->assertJsonPath('candidates.0.matched_child_cari_codes.0', 'SYNTH-CARI-019.KONSINYE')
            ->assertJsonPath('candidates.0.child_cari_accounts.0.mikro_cari_kodu', 'SYNTH-CARI-019.KONSINYE')
            ->assertJsonPath('candidates.0.child_cari_accounts.0.usage_type', 'consignment')
            ->assertJsonPath('candidates.0.child_cari_accounts.1.mikro_cari_kodu', 'SYNTH-CARI-019.TEŞHIR');
        ...
        $this->actingAs($admin)
            ...
            ->assertJsonPath('candidates.0.mikro_cari_kodu', 'SYNTH-CARI-019')
            ->assertJsonPath('candidates.0.search_match', 'child')
            ->assertJsonPath('candidates.0.matched_child_cari_codes.0', 'SYNTH-CARI-019.PROJE');
    ...
    public function test_cari_control_normalizes_contact_and_location_aliases(): void
    {
        ...
        Http::fake([
            'https://n8n.test/*' => Http::response([
                ...
                'rows' => [[
                    ...
                    'sehir' => 'Sentetik Sehir 004 / SYNTHETIC',
                    'ilçe' => 'Cankaya',
        ...
        $this->actingAs($admin)
            ...
            ->assertJsonPath('candidates.0.city', 'Sentetik Sehir 004 / SYNTHETIC')
    ...
    public function test_cari_candidate_title_does_not_duplicate_same_name(): void
    {
        ...
        Http::fake([
            'https://n8n.test/*' => Http::response([
                ...
                'rows' => [[
                    ...
                    'firma_unvani' => 'Sentetik ÇETİN',
                    'firma_unvani_2' => 'Sentetik ÇETİN',
                    'grup' => 'ÇİLİNGİR',
                    'phone' => '5334491851',
                    'city' => 'ADANA',
        ...
        $this->actingAs($admin)
            ...
            ->assertOk()
            ->assertJsonCount(1, 'candidates')
            ->assertJsonPath('candidates.0.display_name', 'Sentetik ÇETİN')
            ->assertJsonPath('candidates.0.mikro_cari_unvan', 'Sentetik ÇETİN')
    ...
    public function test_synthetic_person009_like_partner_can_have_dealer_and_locksmith_roles_in_preview(): void
    {
        $admin = $this->userWithRole('admin', true);
        $this->dataSource('customers_list');
        ...
        Http::fake([
            'https://n8n.test/*' => Http::response([
                ...
                'rows' => [[
                    ...
                    'firma_unvani' => 'SyntheticPerson009 Bayi Çilingir',
                    'grup' => 'BAYİ ÇİLİNGİR',
                    'city' => 'Sentetik Sehir 004 / SYNTHETIC',
                ]],
            ]),
    ...
    public function test_cari_apply_enriches_missing_contact_fields_from_detail_source_and_preserves_existing_values(): void
    {
        ...
        Http::fake([
            'https://n8n.test/*' => Http::response([
                ...
                'rows' => [[
                    ...
                    'cari_il' => 'Sentetik Sehir 004 / SYNTHETIC',
                    'cari_ilce' => 'Cankaya',
        ...
        $this->assertSame('Sentetik Sehir 004 / SYNTHETIC', $partner->city);
    ...
    public function test_cari_control_locksmith_apply_creates_partner_technician_and_link(): void
    {
        ...
        $this->assertSame('Sentetik Sehir 031', $technician->city);
        ...
        $this->assertSame('Sentetik Sehir 031', $link->service_city);
    ...
    public function test_apply_auto_geocode_writes_lat_lng_when_quality_ok(): void
    {
        ...
        Http::fake([
            'https://maps.googleapis.com/maps/api/geocode/json*' => Http::response([
                'status' => 'OK',
                'results' => [[
                    'formatted_address' => 'Sentetik Sehir 031 Organize SyntheticPerson030 Bölgesi, Yunusemre/Sentetik Sehir 031, Türkiye',
                    'geometry' => [
        ...
        $this->actingAs($admin)
            ->postJson('/api/b2b/cari-control/apply', [
                ...
                'candidates' => [[
                    ...
                    'city' => 'Sentetik Sehir 031',
                    'district' => 'Yunusemre',
                    'address' => 'Organize SyntheticPerson030 Bölgesi',
                ]],
            ])
            ->assertOk()
    ...
    public function test_existing_manual_coordinates_not_overwritten_without_override(): void
    {
        ...
        Http::fake([
            'https://maps.googleapis.com/maps/api/geocode/json*' => Http::response([
                'status' => 'OK',
                'results' => [[
                    'formatted_address' => 'Farklı Adres, Sentetik Sehir 031, Türkiye',
                    'geometry' => [
                        'location_type' => 'ROOFTOP',
                        'location' => ['lat' => 38.619099, 'lng' => 27.428921],
                    ],
    ...
    public function test_distinct_synthetic_technician_creation_does_not_reuse_existing_technician(): void
    {
        (new B2BPartnerPermissionSeeder)->run();
        $admin = $this->userWithRole('admin', true);
        $partnerFixture = TechnicalServiceSyntheticDataFactory::locksmith(4);
        ...
        $partner = $this->partner([
            ...
            'display_name' => $partnerFixture['display_name'],
            'mikro_cari_kodu' => $partnerFixture['cari_code'],
            'mikro_cari_unvan' => $partnerFixture['cari_title'],
            'city' => $partnerFixture['city'],
            'district' => 'Synthetic District',
        ]);
        $existingTechnician = $this->technician([
            'name' => $existingFixture['name'],
            'display_name' => $existingFixture['display_name'],
            'first_name' => $existingFixture['first_name'],
            'last_name' => $existingFixture['last_name'],
            'phone' => $existingFixture['phone_e164'],
            'city' => $existingFixture['city'],
            'district' => null,
            'address' => null,
            'mikro_cari_kodu' => $partnerFixture['cari_code'],
            'cari_code' => $partnerFixture['cari_code'],
            'latitude' => $existingCoordinates['latitude'],
            'longitude' => $existingCoordinates['longitude'],
            'start_latitude' => $existingCoordinates['start_latitude'],
            'start_longitude' => $existingCoordinates['start_longitude'],
            'location_source' => 'manual',
        ]);
        B2BPartnerTechnician::query()->create([
            'partner_id' => $partner->id,
            'technical_service_technician_id' => $existingTechnician->id,
        ...
        $this->actingAs($admin)
            ->postJson('/api/b2b/cari-control/apply', [
                ...
                'candidates' => [[
                    ...
                    'display_name' => $partnerFixture['display_name'],
                    'mikro_cari_unvan' => $partnerFixture['cari_title'],
                    'phone' => $partnerFixture['phone_e164'],
                    'city' => $partnerFixture['city'],
                    'district' => 'Synthetic District',
                    'address' => $partnerFixture['address'],
                ]],
            ])
            ->assertOk()
            ->assertJsonPath('items.0.technician_sync.status', 'technician_created')
            ->assertJsonPath('items.0.technician_sync.ignored_technician_id', $existingTechnician->id)
            ->assertJsonPath('items.0.technician_sync.ignored_technician_reason', 'different_person_same_cari_or_phone');

        $existingTechnician->refresh();
        $this->assertSame($existingFixture['name'], $existingTechnician->name);
        $this->assertSame($existingFixture['display_name'], $existingTechnician->display_name);
        $this->assertSame($existingFixture['city'], $existingTechnician->city);
        $this->assertNull($existingTechnician->address);
        $this->assertEquals($existingCoordinates['latitude'], $existingTechnician->latitude);
        $this->assertEquals($existingCoordinates['longitude'], $existingTechnician->longitude);
        $this->assertSame($partnerCount, B2BPartner::query()->count());

        $createdTechnician = TechnicalServiceTechnician::query()
            ->where('name', $partnerFixture['display_name'])
            ->firstOrFail();

        $this->assertNotSame($existingTechnician->id, $createdTechnician->id);
        $this->assertSame($partnerFixture['city'], $createdTechnician->city);
        $this->assertSame(2, B2BPartnerTechnician::query()
            ->where('partner_id', $partner->id)
            ->where('active', true)
            ->count());
        ...
        $links = $this->actingAs($admin)
            ...
            ->assertOk()
            ->json('items');
        $existingPayload = collect($links)->firstWhere('technical_service_technician_id', $existingTechnician->id);
        $this->assertSame($existingFixture['name'], data_get($existingPayload, 'technician.name'));
        $this->assertSame($existingFixture['city'], data_get($existingPayload, 'technician.city'));
        $this->assertEquals($existingCoordinates['latitude'], data_get($existingPayload, 'technician.latitude'));

        $searchItems = $this->actingAs($admin)
            ->getJson('/api/b2b/locksmith-technicians?search=027')
            ->assertOk()
            ->json('items');

        $this->assertNotNull(collect($searchItems)->firstWhere('id', $existingTechnician->id));
    }
    ...
    public function test_cari_control_existing_partner_preview_does_not_target_different_linked_technician(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertJsonPath('items.0.technician_action', 'create_technician')
            ->assertJsonPath('items.0.ignored_technician_id', $existingTechnician->id)
            ->assertJsonPath('items.0.ignored_technician_reason', 'different_person_same_cari_or_phone');

        $this->assertSame($existingFixture['name'], $existingTechnician->fresh()->name);
        $this->assertSame($existingFixture['city'], $existingTechnician->fresh()->city);
    }
    ...
    public function test_dealer_cari_import_creates_default_scoped_dealer_user_and_child_metadata(): void
    {
        ...
        $this->actingAs($admin)
            ->postJson('/api/b2b/cari-control/apply', [
                ...
                'selected_capabilities' => [B2BPartner::TYPE_DEALER],
                'candidates' => [[
                    'mikro_cari_kodu' => 'SYNTH-CARI-004',
                    'display_name' => 'Sentetik Cilingir Ornek 004',
                    'mikro_cari_unvan' => 'Sentetik Cilingir Ornek 004',
                    'phone' => '+905551112233',
                    'email' => 'SyntheticPerson009@example.test',
                    'city' => 'Sentetik Sehir 004 / SYNTHETIC',
                    'district' => 'Cankaya',
                    'address' => 'Bayi Mahallesi No:1',
                    'tax_no' => '9988776655',
                    'tax_office' => 'Sentetik Sehir 004 / SYNTHETIC VD',
                    'child_cari_accounts' => [[
                        'mikro_cari_kodu' => 'SYNTH-CARI-004.KONSINYE',
                        'mikro_cari_unvan' => 'Sentetik Cilingir Ornek 004 KONSINYE',
                        'usage_type' => 'consignment',
                        'invoice_usage_note' => 'Konsinye siparisi/faturasi icin bu alt cari hesabi kullanilacak.',
                    ], [
                        'mikro_cari_kodu' => 'SYNTH-CARI-004.TESHIR',
                        'mikro_cari_unvan' => 'Sentetik Cilingir Ornek 004 TESHIR',
                        'usage_type' => 'showroom',
                    ], [
                        'mikro_cari_kodu' => 'SYNTH-CARI-004.PROJE',
                        'mikro_cari_unvan' => 'Sentetik Cilingir Ornek 004 PROJE',
                        'usage_type' => 'project',
                    ]],
                ]],
            ])
        ...
        $this->assertSame('Sentetik Sehir 004 / SYNTHETIC VD', $partner->metadata['invoice_profile']['tax_office']);
        $this->assertSame('SYNTH-CARI-004.KONSINYE', $partner->metadata['shipping_profile']['consignment_cari_kodu']);
        $this->assertSame('SYNTH-CARI-004.TESHIR', $partner->metadata['shipping_profile']['showroom_cari_kodu']);
        $this->assertSame('SYNTH-CARI-004.PROJE', $partner->metadata['shipping_profile']['project_cari_kodu']);
        ...
        $this->actingAs($admin)
            ->postJson('/api/b2b/cari-control/apply', [
                ...
                'candidates' => [[
                    ...
                    'display_name' => 'SyntheticPerson009 SyntheticPerson010',
                ]],
            ])
            ->assertOk()
    ...
    public function test_cari_control_apply_updates_snapshot_adds_capability_and_marks_review(): void
    {
        ...
        $this->assertSame('Sentetik Sehir 009', $partner->city);
    ...
    public function test_locksmith_and_hybrid_partner_admin_scopes_are_partner_synthetic_person032(): void
    {
        (new B2BPartnerPermissionSeeder)->run();
        $admin = $this->userWithRole('admin', true);
        ...
        $hybrid = $this->partner([
            ...
            'mikro_cari_kodu' => 'SYNTH-CARI-019',
        ]);
    ...
    public function test_finalized_earning_completed_payable_srv003_partner_earnings_tab_partner_job_card_not_pending_estimated_after_ops_final_approval(): void
    {
        ...
        $technician = $this->technician(['name' => 'Sentetik Cilingir Ornek 038']);
        B2BPartnerTechnician::query()->create([
            'partner_id' => $partner->id,
            'technical_service_technician_id' => $technician->id,
            'relationship_type' => 'owner',
    ...
    public function test_spare_part_request_lifecycle_blocks_completion_and_creates_srv_child(): void
    {
        ...
        $this->actingAs($portalUser)
            ->postJson("/api/partner/service-jobs/{$job->id}/support-request", [
                'type' => 'spare_part',
                'description' => 'SyntheticPerson038 karşılığı değişmeli.',
    ...
    public function test_partner_actions_keep_real_actor_and_ops_can_resolve_support_price_and_part_requests(): void
    {
        ...
        $this->actingAs($portalUser)
            ->postJson("/api/partner/service-jobs/{$job->id}/support-request", [
                'type' => 'spare_part',
                'description' => 'SyntheticPerson038 karşılığı değiştirilmeli.',
                'product_name' => 'SyntheticPerson038 karşılığı',
                'quantity' => 1,
            ])
            ->assertOk()
            ->assertJsonPath('status', TechnicalServicePartnerJobAction::STATUS_OPS_REVIEW);
        ...
        $this->assertStringContainsString('SyntheticPerson038 karşılığı', $partBody);
        $this->assertStringContainsString('SyntheticPerson038 karşılığı değiştirilmeli.', $partBody);
    ...
    public function test_technical_service_full_locksmith_part_return_journey(): void
    {
        ...
        $job = $this->serviceRequestForTechnician($technician, 'MRN-E2E-FLOW-PART-RETURN', [
            ...
            'product_name' => 'E2E Test SyntheticPerson038',
    ...
    public function test_technical_service_assignment_creates_assignment_offer_job_card_link_for_portal(): void
    {
        ...
        $job = TechnicalServiceRequest::query()->create([
            ...
            'product_name' => 'Offer SyntheticPerson038',
    ...
    public function test_technical_service_assignment_requires_final_earning_confirmation_when_final_amounts_are_sent(): void
    {
        ...
        $job = TechnicalServiceRequest::query()->create([
            ...
            'product_name' => 'Final Onay SyntheticPerson038',
            'service_type' => 'Montaj',
            'status' => 'Yeni',
    ...
    public function test_partner_portal_earning_summary_corrects_stale_earning_message_total(): void
    {
        ...
        $job = TechnicalServiceRequest::query()->create([
            ...
            'product_name' => 'Stale SyntheticPerson038',
    ...
    private function mountSessionForServiceRequest(TechnicalServiceRequest $request): TechnicalServiceMountSession
    {
        ['link' => $link] = TechnicalServiceQrLink::createPreSaleProductLink([
            ...
            'product_name' => $request->product_name ?: 'Test SyntheticPerson038',
