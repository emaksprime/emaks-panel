...
class TechnicalServiceTechnicianGeocodeTest extends TestCase
{
    ...
    public function test_query_builder_uses_expected_priority_and_skips_empty_addresses(): void
    {
        ...
        $plusCodeTechnician = new TechnicalServiceTechnician([
            'name' => 'Plus Code Usta',
            'google_plus_code' => 'SYNTH-LOCATION-035',
    ...
    public function test_coordinate_validation_command_clears_only_impossible_coordinates(): void
    {
        ...
        $outOfRangePayload = TechnicalServiceSyntheticDataFactory::locksmith(76, [
            'latitude' => '91.0000000',
            'longitude' => '181.0000000',
            'start_latitude' => '91.0000000',
            'start_longitude' => '181.0000000',
        ]);
        unset($outOfRangePayload['synthetic']);
        $outOfRange = TechnicalServiceTechnician::query()->create($outOfRangePayload);
        ...
        $this->assertNull($outOfRange->refresh()->latitude);
        $this->assertNull($outOfRange->longitude);
    }
    ...
    {
        $goodPayload = TechnicalServiceSyntheticDataFactory::locksmith(70, array_merge(
            TechnicalServiceSyntheticDataFactory::coordinate(70),
            ['location_source' => 'synthetic_fixture'],
        ));
        unset($goodPayload['synthetic']);
        $good = TechnicalServiceTechnician::query()->create($goodPayload);

        $reviewPayload = TechnicalServiceSyntheticDataFactory::locksmith(71, array_merge(
            TechnicalServiceSyntheticDataFactory::coordinate(71),
            ['needs_review' => true],
        ));
        unset($reviewPayload['synthetic']);
        TechnicalServiceTechnician::query()->create($reviewPayload);

        $nonLocksmith = TechnicalServiceSyntheticDataFactory::locksmith(72, array_merge(
            TechnicalServiceSyntheticDataFactory::coordinate(72),
            ['technician_type' => 'technician'],
        ));
        unset($nonLocksmith['synthetic']);
        TechnicalServiceTechnician::query()->create($nonLocksmith);

        foreach ([73, 74, 75] as $sequence) {
            $duplicate = TechnicalServiceSyntheticDataFactory::locksmith($sequence, [
                'latitude' => '38.9637450',
                'longitude' => '35.2433220',
            ]);
            unset($duplicate['synthetic']);
            TechnicalServiceTechnician::query()->create($duplicate);
        }

        $outputPath = storage_path('app/private/technical-service/synthetic-coordinate-export.json');
        ...
        $this->assertSame('synthetic_fixture', $data['items'][0]['geocode_quality']);
        $this->assertArrayNotHasKey('phone', $data['items'][0]);
        $this->assertNotContains($reviewPayload['source_key'], array_column($data['items'], 'source_key'));
        $this->assertNotContains($nonLocksmith['source_key'], array_column($data['items'], 'source_key'));
        unlink($outputPath);
    }

    public function test_coordinate_import_defaults_to_dry_run_and_apply_is_idempotent(): void
    {
        $sourceFixture = TechnicalServiceSyntheticDataFactory::locksmith(80);
        unset($sourceFixture['synthetic']);
        $sourceKeyTechnician = TechnicalServiceTechnician::query()->create($sourceFixture);

        $legacyFixture = TechnicalServiceSyntheticDataFactory::locksmith(81, ['source_key' => null]);
        unset($legacyFixture['synthetic']);
        $phoneCityTechnician = TechnicalServiceTechnician::query()->create($legacyFixture);

        $path = storage_path('app/private/technical-service/synthetic-coordinate-import.json');
        File::ensureDirectoryExists(dirname($path));
        file_put_contents($path, json_encode([
            'synthetic' => true,
            'items' => [
                TechnicalServiceSyntheticDataFactory::coordinate(80),
                TechnicalServiceSyntheticDataFactory::coordinate(81, [
                    'source_key' => 'synthetic-legacy-coordinate-key',
        ...
        [$dryRun, $dryRunOutput] = $this->callArtisan('technical-service:import-technician-coordinates', ['--source' => $path]);
        $this->assertSame(0, $dryRun, $dryRunOutput);
        $this->assertNull($sourceKeyTechnician->fresh()->latitude);
        $this->assertNull($phoneCityTechnician->fresh()->latitude);

        [$first, $firstOutput] = $this->callArtisan('technical-service:import-technician-coordinates', [
            '--source' => $path,
            '--apply' => true,
        ]);
        $this->assertSame(0, $first, $firstOutput);
        $this->assertStringContainsString('update: 2', $firstOutput);

        [$second, $secondOutput] = $this->callArtisan('technical-service:import-technician-coordinates', [
            '--source' => $path,
            '--apply' => true,
        ]);
        $this->assertSame(0, $second, $secondOutput);
        $this->assertStringContainsString('update: 0', $secondOutput);
        $this->assertStringContainsString('delete: 0', $secondOutput);
        unlink($path);
    }

    public function test_coordinate_import_conflict_rolls_back_all_rows(): void
    {
        $firstFixture = TechnicalServiceSyntheticDataFactory::locksmith(90);
        $secondFixture = TechnicalServiceSyntheticDataFactory::locksmith(91);
        unset($firstFixture['synthetic'], $secondFixture['synthetic']);
        $first = TechnicalServiceTechnician::query()->create($firstFixture);
        $second = TechnicalServiceTechnician::query()->create($secondFixture);

        $path = storage_path('app/private/technical-service/synthetic-coordinate-conflict.json');
        File::ensureDirectoryExists(dirname($path));
        file_put_contents($path, json_encode(TechnicalServiceSyntheticDataFactory::dataset([
            TechnicalServiceSyntheticDataFactory::coordinate(90),
            TechnicalServiceSyntheticDataFactory::coordinate(91, ['source_key' => null]),
        ]), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));

        $exit = Artisan::call('technical-service:import-technician-coordinates', [
            '--source' => $path,
            '--apply' => true,
        ]);
        ...
        $this->assertNull($second->fresh()->latitude);
        unlink($path);
    }

    public function test_coordinate_seeder_requires_explicit_private_path(): void
    {
        try {
            Artisan::call('db:seed', ['--class' => 'TechnicalServiceTechnicianCoordinateSeeder', '--force' => true]);
            $this->fail('Coordinate seeder unexpectedly accepted a missing explicit path.');
