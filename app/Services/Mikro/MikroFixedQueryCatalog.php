...
{
    /**
     * Every template is owned by the server and derived from the existing V17/n8n contracts.
    ...
    private const QUERIES = [
        ...
        'stock.availability' => [
            'sql' => "SELECT TOP 1 sto.sto_kod AS stock_code, sto.sto_isim AS stock_name, CAST(ISNULL(dbo.fn_DepodakiMiktar(sto.sto_kod, 1, GETDATE()), 0) + ISNULL(dbo.fn_DepodakiMiktar(sto.sto_kod, 5, GETDATE()), 0) AS decimal(18,2)) AS available_quantity FROM dbo.STOKLAR AS sto WITH (NOLOCK) WHERE LTRIM(RTRIM(sto.sto_kod)) = [[stock_code]]",
    ...
    public function definition(string $queryId): array
    {
        ...
        return ['query_id' => $queryId, ...$definition];
    }
