...
class ServiceJobConfirmationController extends Controller
{
    ...
    public function show(string $token): Response
    {
        ...
        if (! $confirmation instanceof TechnicalServiceCustomerConfirmation) {
            return response($this->invalidHtml(), 410);
        }

        $state = $this->state($confirmation);

        return response($this->html($confirmation, $state), $state === 'expired' ? 410 : 200);
    }
    ...
    public function approve(Request $request, string $token): Response
    {
        ...
        if ($state !== 'pending') {
            return response($this->html($confirmation->refresh(), 'expired'), 410);
        }
    ...
    private function state(TechnicalServiceCustomerConfirmation $confirmation): string
    {
        ...
        if ($confirmation->status !== TechnicalServiceCustomerConfirmation::STATUS_PENDING) {
            return 'expired';
        }
    ...
    private function html(TechnicalServiceCustomerConfirmation $confirmation, string $state): string
    {
        ...
        return '<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'
            ...
            .e($title).
            '</h1><p class="muted">'.($approved || $rejected || $expired ? 'Bu sayfayı kapatabilirsiniz.' : 'Montaj işleminizin tamamlandığını onaylamak için bilgileri kontrol edin.').'</p><div class="grid"><div class="row"><strong>MRN</strong><br>'
            .e((string) $job->mrn).
    ...
    private function invalidHtml(): string
    {
        return '<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Onay bağlantısı geçerli değil</title><style>*,*::before,*::after{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,Segoe UI,sans-serif;background:#f3f7fb;color:#0f172a}.wrap{width:100%;max-width:680px;margin:40px auto;padding:24px}.card{background:white;border:1px solid #dbe4ef;border-radius:24px;padding:24px;box-shadow:0 24px 70px rgba(15,23,42,.08)}h1{font-size:28px;margin:0 0 12px}.muted{color:#64748b}.danger{border:1px solid #fecdd3;background:#fff1f2;color:#be123c;border-radius:16px;padding:14px;font-weight:700}@media(max-width:520px){.wrap{margin:0;padding:16px}.card{border-radius:20px;padding:18px}h1{font-size:24px}}</style></head><body><main class="wrap"><section class="card"><h1>Onay bağlantısı geçerli değil</h1><p class="muted">Bu sayfayı kapatabilirsiniz.</p><div class="danger"><strong>Bu onay bağlantısı artık geçerli değil.</strong><br>Yeni bağlantı gerekiyorsa operasyon ekibimizle iletişime geçin.</div></section></main></body></html>';
    }
}
