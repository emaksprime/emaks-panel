...
class B2BPortalPreviewController extends Controller
{
    ...
    public function __invoke(Request $request, B2BPartner $partner): Response
    {
        ...
        return Inertia::render('panel/b2b/portal-preview', [
            ...
            'partnerPortal' => [
                ...
                'allowed' => true,
                'deniedMessage' => null,
                'partners' => [$this->partnerSummary($partner)],
     ...
     */
    private function serviceJobsFor(B2BPartner $partner): array
    {
    ...
    {
        return [
            'orders' => 'Bayi sipariş entegrasyonu sonraki fazda Mikro read-only datasource ile bağlanacak.',
            'stock' => 'Bayi stok entegrasyonu sonraki fazda Mikro read-only datasource ile bağlanacak.',
            'finance' => 'Cari ve risk özeti sonraki fazda read-only cari datasource ile bağlanacak.',
            'service' => $partner->hasCapability(B2BPartner::TYPE_LOCKSMITH)
                ? 'Servis işleri bağlı owner/field usta kapsamından okunur.'
                : 'Bu partner için çilingir portal kimliği aktif değil.',
        ];
    }
}
