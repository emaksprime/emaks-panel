        ...
        void Promise.allSettled([
            ...
            .then(([stockResult, settingsResult]) => {
                ...
                if (stockResult.status === 'fulfilled') {
                    setStockData({
                        rows: Array.isArray(stockResult.value?.rows) ? stockResult.value.rows : [],
                        columns: Array.isArray(stockResult.value?.columns) ? stockResult.value.columns : [],
                        queryMeta: stockResult.value?.queryMeta ?? null,
                    });
                    setError(null);
                } else {
                    setStockData({ rows: [], columns: [], queryMeta: null });
                    setError(stockResult.reason?.message ?? 'Stok verisi alÄ±namadÄ±.');
                }

                if (settingsResult.status === 'fulfilled') {
                    setSettingsData({
                        rows: Array.isArray(settingsResult.value?.rows) ? settingsResult.value.rows : [],
                        can_manage: Boolean(settingsResult.value?.can_manage),
                    });
                } else {
                    setSettingsData({ rows: [], can_manage: false });
                }
            })
            .catch((requestError) => {
                if (cancelled) {
