...
export default function AdminDataSources() {
    ...
    return (
        <AdminFrame title="Datasource Yonetimi">
                ...
                <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
                    {data.dataSources.map((source) => (
                        <button key={source.id} type="button" onClick={() => edit(source)} className="grid w-full gap-1 border-b border-slate-100 px-4 py-3 text-left">
                            ...
                            <span className="text-sm text-slate-500">{source.code} - {source.db_type} - {source.active ? 'active' : 'inactive'}</span>
                        </button>
                    ))}
                </div>
                <form onSubmit={save} className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                    <input className="rounded-md border border-slate-200 px-3 py-2" placeholder="code" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} />
                    <input className="rounded-md border border-slate-200 px-3 py-2" placeholder="name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} />
                    ...
                    <input className="rounded-md border border-slate-200 px-3 py-2" placeholder="allowed params: date_from, date_to" value={allowedParamsText} onChange={(event) => setAllowedParamsText(event.target.value)} />
                    <textarea className="min-h-56 rounded-md border border-slate-200 px-3 py-2 font-mono text-xs" placeholder="query_template" value={form.query_template} onChange={(event) => setForm({ ...form, query_template: event.target.value })} />
                    <textarea className="min-h-20 rounded-md border border-slate-200 px-3 py-2" placeholder="description" value={form.description ?? ''} onChange={(event) => setForm({ ...form, description: event.target.value })} />
