...
export type PendingPaymentLinkSurface =
    | 'payment-modal'
    | 'technical-service'
    | 'root-mrn'
    | 'child-srv'
...
export type PendingPaymentLinkActionPayment = Pick<
    ServiceRequestExtraMountPayment,
    | 'id'
    | 'status'
    | 'canonical_url'
    | 'payment_url'
...
export const canonicalPendingPaymentUrl = (
    payment: PendingPaymentLinkActionPayment,
): string =>
    String(
        payment.canonical_url ?? payment.copy_url ?? payment.payment_url ?? '',
    ).trim();

export const paymentLinkSendDisabledReason = (
    payment: PaymentLinkSendContext,
): string | null => {
...
export function PaymentLinkSendDialog({
    ...
    const fields = [
        ...
        [
            requestLabel,
            payment.request_code || payment.root_mrn || requestReference,
        ],
        ['Tahsilat amacı', payment.purpose_label || payment.purpose || '-'],
        [
            'Tutar',
            payment.amount_label ||
                `${payment.amount.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${payment.currency || 'TRY'}`,
        ],
        ...
        [
            'Hedef modu',
            payment.message_target_mode === 'test'
                ? 'Test yönlendirmesi'
                : 'Gerçek müşteri',
        ],
    ...
    return (
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Ödeme bağlantısını müşteriye gönder"
        >
            <div className="grid max-h-[92vh] w-full max-w-2xl gap-4 overflow-y-auto rounded-lg bg-white p-5 shadow-xl">
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <h2 className="text-base font-semibold text-slate-950">
                            Ödeme bağlantısını müşteriye gönder
                        </h2>
                        <p className="mt-1 text-sm text-slate-600">
                            Seçilen canonical ödeme kaydı ve gönderim hedefi
                        </p>
                    </div>
                    <Button
                        type="button"
                ...
                <dl
                    data-testid="payment-link-send-context"
                ...
                >
                    {fields.map(([label, value]) => (
                        <div key={label} className="min-w-0 bg-white px-3 py-2">
                            ...
                            <dd className="mt-1 text-sm font-semibold break-words text-slate-900">
                                {value}
                            </dd>
                        </div>
                    ))}
                ...
                <div className="min-w-0 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    ...
                    <p
                        data-testid="payment-link-send-canonical-url"
                        className="mt-1 text-sm font-semibold break-all text-slate-900"
                    >
                        {canonicalUrl || '-'}
                    </p>
                </div>
                ...
                {isResend ? (
                    <label className="grid gap-1 text-sm font-medium text-slate-800">
                        ...
                        <Input
                            value={resendReason}
                            onChange={(event) =>
                                onResendReasonChange(event.target.value)
                            }
                ...
                {sendDisabledReason ? (
                    <p
                        data-testid="payment-link-send-disabled-reason"
                        role="status"
                ...
                {resultMessage ? (
                    <p
                        data-testid="payment-link-send-result"
                        role="status"
                ...
                {errorMessage ? (
                    <p
                        data-testid="payment-link-send-error"
                        role="alert"
                ...
                <div className="flex flex-wrap justify-end gap-2">
                    ...
                    <Button
                        data-testid="payment-link-send-confirm"
                        type="button"
                        disabled={Boolean(sendDisabledReason) || busy}
...
export function PendingPaymentLinkActions({
    ...
    const canOpen =
        isPending &&
        canonicalUrl !== '' &&
        (payment.can_open ?? payment.can_open_payment_url ?? true);
    const canCopy =
        isPending &&
        canonicalUrl !== '' &&
        (payment.can_copy ?? payment.can_copy_payment_url ?? true) &&
        Boolean(onCopy);
    const canSend =
        isPending &&
        canonicalUrl !== '' &&
        (payment.can_send ?? true) &&
        Boolean(onSend) &&
        !sendDisabledReason;
    const canCheck =
        isPending &&
        (payment.can_check ?? payment.is_external_provider ?? false) &&
        Boolean(onCheck);
    const canCancel =
        isPending &&
        (payment.can_cancel ?? payment.can_cancel_payment ?? true) &&
        Boolean(onCancel);
    const disabledReason =
        payment.disabled_reason ??
    ...
    return (
        <div
            data-testid={testId}
            data-payment-link-surface={surface}
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                <Button
                    ...
                    title={
                        !canCopy
                            ...
                            : 'Canonical ödeme bağlantısını kopyala'
                    }
                ...
                <Button
                    ...
                    title={
                        ...
                        (!canSend
                            ? (disabledReason ?? undefined)
                            : 'Ödeme bağlantısını müşteriye WhatsApp ve SMS ile gönder')
                    }
                ...
                <Button
                    ...
                    title={
                        !canCheck
                            ? (disabledReason ?? undefined)
                            : 'Provider ödeme durumunu kontrol et'
                    }
                ...
                <Button
                    ...
                    title={
                        !canCancel
                            ? (disabledReason ?? undefined)
                            : 'Bekleyen ödeme bağlantısını iptal et'
                    }
